...
import {writeUtil} from 'usdao-sdkss';
import {getNetwork} from '../../blockchain/sdkFunction';


const writeFunction = new writeUtil(process.env.REACT_APP_INFURA_LINK);

console.log("writeFunction", writeFunction);
(async ()=>{
	//console.log("address", await writeFunction.readUtil.userAddress());
})()

export const netWork = async () => {
	console.log(111111111111111);

	console.log("address", await writeFunction.readUtil.provider);

}

export const getAccoundDetails = async() =>{
	const address = await writeFunction.readUtil.userAddress();
	const ethBalance = await writeFunction.readUtil.balaceOfEther();
	const chainId= await writeFunction.readUtil.getChainId();
	const usmBalance = await writeFunction.readUtil.balanceOf('usm', address);
	const assetBalance = await writeFunction.readUtil.balanceOf('fum', address);
	const CollateralBalance = await writeFunction.readUtil.getCollateral();
	const bufferEth = await writeFunction.readUtil.getEthBuffer();
	const debtRatio = await writeFunction.readUtil.getDebtRatio();
	console.log("CollateralBalance", CollateralBalance,"debtRatio", debtRatio);

	return {
		networkType: await getNetwork(chainId),
		address,
		balance: ethBalance,
		usmBalanceFormatted : usmBalance,
		assetBalance,
		etherBalanceFormatted : ethBalance
